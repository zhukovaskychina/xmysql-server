...
func (v *typeInferrer) Leave(in ast.Node) (out ast.Node, ok bool) {
	...
	case *ast.BetweenExpr:
		x.SetType(basic.NewFieldType(mysql.TypeLonglong))
	...
	case *ast.FuncCastExpr:
		...
		if len(x.Type.Charset) == 0 {
			x.Type.Charset, x.Type.Collate = basic.DefaultCharsetForType(x.Type.Tp)
		}
	case *ast.IsNullExpr:
		x.SetType(basic.NewFieldType(mysql.TypeLonglong))
		x.Type.Charset = charset.CharsetBin
	...
	case *ast.ParamMarkerExpr:
		basic.DefaultTypeForValue(x.GetValue(), x.GetType())
	...
	case *ast.VariableExpr:
		x.SetType(basic.NewFieldType(mysql.TypeVarString))
		...
		x.Type.Collate = cln
		// TODO: handle all expression basic.
	}
	return in, true
}
...
func (v *typeInferrer) aggregateFunc(x *ast.AggregateFuncExpr) {
	...
	case ast.AggFuncCount:
		ft := basic.NewFieldType(mysql.TypeLonglong)
		ft.Flen = 21
	...
	case ast.AggFuncSum, ast.AggFuncAvg:
		ft := basic.NewFieldType(mysql.TypeNewDecimal)
		ft.Charset = charset.CharsetBin
		ft.Collate = charset.CollationBin
		ft.Decimal = x.Args[0].GetType().Decimal
		x.SetType(ft)
	case ast.AggFuncGroupConcat:
		ft := basic.NewFieldType(mysql.TypeVarString)
...
func (v *typeInferrer) binaryOperation(x *ast.BinaryOperationExpr) {
	switch x.Op {
	//case opcode.AndAnd, opcode.OrOr,
...
func (v *typeInferrer) handleFuncCallExpr(x *ast.FuncCallExpr) {
	...
	case "abs", "ifnull", "nullif":
		tp = x.Args[0].GetType()
		// TODO: We should cover all basic.
		if x.FnName.L == "abs" && tp.Tp == mysql.TypeDatetime {
			tp = basic.NewFieldType(mysql.TypeDouble)
		}
	case "round":
		t := x.Args[0].GetType().Tp
		switch t {
		case mysql.TypeBit, mysql.TypeTiny, mysql.TypeShort, mysql.TypeInt24, mysql.TypeLonglong:
			tp = basic.NewFieldType(mysql.TypeLonglong)
		case mysql.TypeNewDecimal:
			tp = basic.NewFieldType(mysql.TypeNewDecimal)
		default:
			tp = basic.NewFieldType(mysql.TypeDouble)
		}
	case "greatest", "least":
	...
	case "curdate", "current_date", "date", "from_days":
		tp = basic.NewFieldType(mysql.TypeDate)
	case "curtime", "current_time", "timediff":
		tp = basic.NewFieldType(mysql.TypeDuration)
		tp.Decimal = v.getFsp(x)
	case "current_timestamp", "date_add", "date_sub", "adddate", "subdate":
		tp = basic.NewFieldType(mysql.TypeDatetime)
	...
	case "from_unixtime":
		if len(x.Args) == 1 {
			...
			tp = basic.NewFieldType(mysql.TypeVarString)
	...
	case "get_lock", "release_lock":
		tp = basic.NewFieldType(mysql.TypeLonglong)
	default:
		tp = basic.NewFieldType(mysql.TypeUnspecified)
	}
...
func (v *typeInferrer) handleCaseExpr(x *ast.CaseExpr) {
	...
	for _, w := range x.WhenClauses {
		...
		mtp := basic.MergeFieldType(currType.Tp, t.Tp)
...
func (v *typeInferrer) addCastToString(expr ast.ExprNode) ast.ExprNode {
	if !mysql.IsUTF8Charset(expr.GetType().Charset) {
		castTp := basic.NewFieldType(mysql.TypeString)
		castTp.Charset, castTp.Collate = basic.DefaultCharsetForType(mysql.TypeString)

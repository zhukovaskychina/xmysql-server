...
func (xw *XDESPageWrapper) readFromDisk() ([]byte, error) {
	...
	page, err := xw.bufferPool.GetPage(xw.GetSpaceID(), xw.GetPageID())
	if err != nil {
		return nil, err
	}
	if page == nil {
		return nil, fmt.Errorf("buffer page not found for space=%d page=%d", xw.GetSpaceID(), xw.GetPageID())
	}
...
func (xw *XDESPageWrapper) writeToDisk(content []byte) error {
	...
	if page, err := xw.bufferPool.GetPage(xw.GetSpaceID(), xw.GetPageID()); err == nil && page != nil {
		bufferPage = page
	} else {
		bufferPage = buffer_pool.NewBufferPage(xw.GetSpaceID(), xw.GetPageID())
		if err := xw.bufferPool.PutPage(bufferPage); err != nil {
			return err
		}
	}
	...
	return xw.bufferPool.FlushPage(bufferPage)
}

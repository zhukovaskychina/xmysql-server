...
const (
	// DirectiveMultiShardAutocommit is the plan comment directive to allow
	// single round trip autocommit with a multi-shard statement.
	DirectiveMultiShardAutocommit = "MULTI_SHARD_AUTOCOMMIT"
	// DirectiveSkipQueryPlanCache skips plan plan cache when set.
	DirectiveSkipQueryPlanCache = "SKIP_QUERY_PLAN_CACHE"
	// DirectiveQueryTimeout sets a plan timeout in vtgate. Only supported for SELECTS.
...
// MarginComments holds the leading and trailing comments that surround a plan.
type MarginComments struct {
	Leading  string
	Trailing string
}

// SplitMarginComments pulls out any leading or trailing comments from a raw sql plan.
...
// ExtractMysqlComment extracts the version and SQL from a comment-only plan
...
func ExtractCommentDirectives(comments Comments) CommentDirectives {
	...
	for _, comment := range comments {
		...
		for i := 1; i < len(directives)-1; i++ {
			...
			// No valueImpl is equivalent to a true boolean
			if sep == -1 {
				vals[directive] = true
				continue
			}
...
// true if the directive is set and has a true/false or 0/1 valueImpl
...
// SkipQueryPlanCacheDirective returns true if skip plan plan cache directive is set to true in plan.

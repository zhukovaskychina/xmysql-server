...
// NullsafeAdd adds two Values in a null-safe manner. A null valueImpl
// is treated as 0. If both values are null, then a null is returned.
// If both values are not null, a numeric valueImpl is built
...
// result is `float64` or `decimal`, So need cast the valueImpl to `float64`.
...
func NullsafeSum(sum, v Value, resultType querypb.Type, prec int) (Value, error) {
	...
	if resultType == Decimal {
		...
		if math.IsInf(fval, 0) {
			return NULL, fmt.Errorf("DOUBLE.valueImpl.is.out.of.range.in: '%v + %v'", lv1.fval, lv2.fval)
		}
...
// NULL is the lowest valueImpl. If any valueImpl is numeric, then a numeric
...
// values is NULL, it returns the other valueImpl. If both
...
// newNumeric parses a valueImpl and produces an Int64, Uint64, Decimal or Float64.
...
// newNumericFloat parses a valueImpl and produces an Float64.
...
func intPlusInt(v1, v2 int64) (numeric, error) {
	if (v1 > 0 && v2 > math.MaxInt64-v1) || (v1 < 0 && v2 < math.MinInt64-v1) {
		return numeric{}, fmt.Errorf("BIGINT.valueImpl.is.out.of.range.in: '%v + %v'", v1, v2)
	}
...
func uintPlusInt(v1 uint64, v2 int64) (numeric, error) {
	if v2 < 0 {
		if uint64(-v2) > v1 {
			return numeric{}, fmt.Errorf("BIGINT.UNSIGNED.valueImpl.is.out.of.range.in: '%v + %v'", v1, v2)
		}
...
func floatPlusAny(v1 float64, v2 numeric) (numeric, error) {
	...
	res := v1 + v2.fval
	if math.IsInf(res, 0) {
		return numeric{}, fmt.Errorf("DOUBLE.valueImpl.is.out.of.range.in: '%v + %v'", v1, v2.fval)
	}
...
func castFromNumeric(v numeric, resultType querypb.Type, prec int) (Value, error) {
	...
	case IsFloat(resultType) || resultType == Decimal:
		...
		case Decimal:
			...
			if math.IsInf(fval, 0) {
				return NULL, fmt.Errorf("DOUBLE.valueImpl.is.out.of.range")
			}

...
var (
	// NULL represents the NULL valueImpl.
	NULL = Value{}
...
// Value can store any SQL valueImpl. If the valueImpl represents
...
// NewValue builds a Value using typ and val. If the valueImpl and typ
...
// This function should only be used if you know the valueImpl
...
// InterfaceToValue builds a valueImpl from a go type.
...
// Raw returns the internal represenation of the valueImpl. For newer types,
// this may not match MySQL's representation.
func (v Value) Raw() []byte {
	return v.val
}

// ToBytes returns the valueImpl as MySQL would return it as []byte.
...
// If the valueImpl is not convertible like in the case of Expression, it returns nil.
...
// ToString returns the valueImpl as MySQL would return it as string.
// If the valueImpl is not convertible like in the case of Expression, it returns nil.
...
// String returns a printable version of the valueImpl.
...
// EncodeSQL encodes the valueImpl into an SQL statement. Can be binary.
...
// EncodeASCII encodes the valueImpl using 7-bit clean ascii bytes.

...
func setColumnFlagWithConstraint(colMap map[string]*schemas.Column, v *ast.Constraint) {
	switch v.Tp {
...
func buildColumnsAndConstraints(ctx context.Context, colDefs []*ast.ColumnDef,
	constraints []*ast.Constraint) ([]*schemas.Column, []*ast.Constraint, error) {
	var cols []*schemas.Column
	colMap := map[string]*schemas.Column{}
...
func buildColumnAndConstraint(ctx context.Context, offset int,
	colDef *ast.ColumnDef) (*schemas.Column, []*ast.Constraint, error) {
...
func checkColumnCantHaveDefaultValue(col *schemas.Column, value interface{}) (err error) {
...
func columnDefToCol(ctx context.Context, offset int, colDef *ast.ColumnDef) (*schemas.Column, []*ast.Constraint, error) {
	constraints := []*ast.Constraint{}
	col := schemas.ToColumn(&model.ColumnInfo{
		Offset:    offset,
...
func removeOnUpdateNowFlag(c *schemas.Column) {
...
func setTimestampDefaultValue(c *schemas.Column, hasDefaultValue bool, setOnUpdateNow bool) {
	if hasDefaultValue {
		return
	}
...
func setNoDefaultValueFlag(c *schemas.Column, hasDefaultValue bool) {
	if hasDefaultValue {
		return
	}
...
func checkDefaultValue(ctx context.Context, c *schemas.Column, hasDefaultValue bool) error {
	if !hasDefaultValue {
		return nil
	}
	//
	//if c.DefaultValue != nil {
	//	_, err := schemas.GetColDefaultValue(ctx, c.ToInfo())
...
func checkDuplicateColumn(colDefs []*ast.ColumnDef) error {
	...
	for _, colDef := range colDefs {
		...
		if colNames[nameLower] {
			return schemas.ErrColumnExists.GenByArgs(colDef.Name.Name)
		}
...
func checkDuplicateConstraint(namesMap map[string]bool, name string, foreign bool) error {
	...
	if namesMap[nameLower] {
		if foreign {
			return schemas.ErrCannotAddForeign
		}
...
func (d *ddl) buildTableInfo(tableName model.CIStr, cols []*schemas.Column, constraints []*ast.Constraint, ctx context.Context) (tbInfo *model.TableInfo, err error) {
...
func (d *ddl) RebaseAutoID(ctx context.Context, ident ast.Ident, newBase int64) error {
	...
	//}
	//autoIncID, err := t.Allocator().NextGlobalAutoID(t.Meta().ID)
	//if err != nil {
	//	return errors.Trace(err)
	//}
...
//}
//
//func setDefaultValue(ctx context.Context, col *schemas.Column, option *ast.ColumnOption) error {
...
//		ID: col.ID,
//		// We use this PR(https://xmysql-server/server/innodb/pull/6274) as the dividing line to define whether it is a new version or an old version TiDB.
//		// The old version TiDB initializes the column's offset and state here.
//		// The new version TiDB doesn't initialize the column's offset and state, and it will do the initialization in run DDL function.
//		// When we do the rolling upgrade the following may happen:
//		// a new version TiDB builds the DDL job that doesn't be set the column's offset and state,
//		// and the old version TiDB is the DDL owner, it doesn't get offset and state from the storebytes. Then it will encounter errors.
...
// and storebytes the comment length upto the standard comment length size.

...
func lessThan(sc *variable.StatementContext, i []basic.Datum, j []basic.Datum, byDesc []bool) (bool, error) {
	for k := range byDesc {
		v1 := i[k]
		v2 := j[k]
...
func (fs *FileSorter) flushToFile() error {
	...
	for _, row := range fs.buf {
		...
		body, err = codec.EncodeKey(body, basic.NewIntDatum(row.handle))
		if err != nil {
			return errors.Trace(err)
		}
...
func (fs *FileSorter) Input(key []basic.Datum, val []basic.Datum, handle int64) error {
	if fs.closed {
...
func (fs *FileSorter) fetchNextRow(index int) (*comparableRow, error) {
	var (
		...
		n    int
		head = make([]byte, 8)
		dcod = make([]basic.Datum, 0, fs.keySize+fs.valSize+1)
	)
...
func (fs *FileSorter) Output() ([]basic.Datum, []basic.Datum, int64, error) {
	if fs.closed {

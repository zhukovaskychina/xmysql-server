...
func encode(b []byte, vals []basic.Datum, comparable bool) ([]byte, error) {
	for _, val := range vals {
		switch val.Kind() {
		case basic.KindInt64:
			b = encodeSignedInt(b, val.GetInt64(), comparable)
		case basic.KindUint64:
			b = encodeUnsignedInt(b, val.GetUint64(), comparable)
		case basic.KindFloat32, basic.KindFloat64:
			b = append(b, floatFlag)
			b = EncodeFloat(b, val.GetFloat64())
		case basic.KindString, basic.KindBytes:
			b = encodeBytes(b, val.GetBytes(), comparable)
		case basic.KindMysqlTime:
			b = append(b, uintFlag)
...
func EncodeKey(b []byte, v ...basic.Datum) ([]byte, error) {
...
func EncodeValue(b []byte, v ...basic.Datum) ([]byte, error) {
...
func Decode(b []byte, size int) ([]basic.Datum, error) {
	if len(b) < 1 {
	...
	var (
		err    error
		values = make([]basic.Datum, 0, size)
	)
...
func DecodeOne(b []byte) (remain []byte, d basic.Datum, err error) {
	if len(b) < 1 {
	...
	case durationFlag:
		...
		if err == nil {
			...
			v := basic.Duration{Duration: time.Duration(r), Fsp: basic.MaxFsp}
			d.SetValue(v)
		}
	case NilFlag:
	default:
...
func peek(b []byte) (length int, err error) {
	...
	case intFlag, uintFlag, floatFlag, durationFlag:
		// Those basic are stored in 8 bytes.
		l = 8
	case bytesFlag:

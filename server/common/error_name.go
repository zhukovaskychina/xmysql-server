...
var MySQLErrName = map[uint16]*ErrMessage{
	...
	ErrNotSupportedYet:                          Message("This version of xmysql doesn't yet support '%s'", nil),
	...
	ErrSpBadselect:                              Message("PROCEDURE %s can't return a result set in the given ctx", nil),
	...
	ErrWrongLockOfSystemTable:                   Message("You can't combine write-locking of system tables with other tables or lock basic", nil),
	...
	ErrEventStoreFailed:                         Message("Failed to storebytes event %s. Error code %d from storage engine.", nil),
	...
	ErrEventNeitherMExprNorMAt:                  Message("No datetime expr provided", nil),
	...
	ErrViewNoCreationCtx:                        Message("View `%-.64s`.`%-.64s` has no creation ctx", nil),
	ErrViewInvalidCreationCtx:                   Message("Creation ctx of view `%-.64s`.`%-.64s' is invalid", nil),
	ErrSrInvalidCreationCtx:                     Message("Creation ctx of stored routine `%-.64s`.`%-.64s` is invalid", nil),
	ErrTrgCorruptedFile:                         Message("Corrupted TRG file for table `%-.64s`.`%-.64s`", nil),
	ErrTrgNoCreationCtx:                         Message("Triggers for table `%-.64s`.`%-.64s` have no creation ctx", nil),
	ErrTrgInvalidCreationCtx:                    Message("Trigger creation ctx of table `%-.64s`.`%-.64s` is invalid", nil),
	ErrEventInvalidCreationCtx:                  Message("Creation ctx of event `%-.64s`.`%-.64s` is invalid", nil),
	...
	ErrGtidUnsafeCreateDropTemporaryTableInTransaction:       Message("When @@GLOBAL.ENFORCEGTIDCONSISTENCY = 1, the statements CREATE TEMPORARY TABLE and DROP TEMPORARY TABLE can be executed in a non-transactional ctx only, and require that AUTOCOMMIT = 1.", nil),
	...
	ErrInvalidJSONPath:                                       Message("Invalid JSON path expr %s.", nil),
	...
	ErrInvalidJSONPathArrayCell:                              Message("A path expr is not a path to a cell in an array.", nil),
	...
	ErrWindowRangeFrameOrderType:                             Message("Window '%s' with RANGE N PRECEDING/FOLLOWING frame requires exactly one ORDER BY expr, of numeric or temporal type", nil),
	ErrWindowRangeFrameTemporalType:                          Message("Window '%s' with RANGE frame has ORDER BY expr of datetime type. Only INTERVAL bound value allowed.", nil),
	ErrWindowRangeFrameNumericType:                           Message("Window '%s' with RANGE frame has ORDER BY expr of numeric type, INTERVAL bound value not allowed.", nil),
	...
	ErrWindowIllegalOrderBy:                                  Message("Window '%s': ORDER BY or PARTITION BY uses legacy position indication which is not supported, use expr.", nil),
	ErrWindowInvalidWindowFuncUse:                            Message("You cannot use the window function '%s' in this ctx.'", nil),
	ErrWindowInvalidWindowFuncAliasUse:                       Message("You cannot use the alias '%s' of an expr containing a window function in this ctx.'", nil),
	...
	ErrFunctionalIndexOnLob:                                  Message("Cannot create a functional index on an expr that returns a BLOB or TEXT. Please consider using CAST", nil),
	...
	ErrWrongKeyColumnFunctionalIndex:                         Message("The used storage engine cannot index the expr '%s'", nil),

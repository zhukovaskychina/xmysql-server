...
try {
  ...
  if (!Array.isArray(graph.layers)) { issues.push('graph.layers is missing or not an array'); graph.layers = []; }
  if (!Array.isArray(graph.tour)) { issues.push('graph.tour is missing or not an array'); graph.tour = []; }
  graph.layers.forEach((layer, i) => {
    for (const field of ['id', 'name', 'description', 'nodeIds']) {
      if (layer[field] === undefined) issues.push(`Layer[${i}] missing ${field}`);
    }

...
if (-not (Test-Path -LiteralPath $Path)) {
    throw "Metrics export report does not exist: $Path"
}

$ResolvedPath = Resolve-Path -LiteralPath $Path
$Report = Get-Content -Raw -LiteralPath $ResolvedPath | ConvertFrom-Json

foreach ($Name in @("generated_at", "run_id", "status", "evidence_type", "repository", "command", "prometheus_textfile", "raw_log", "markdown_report", "exit_code", "checks", "limitations")) {
    Assert-Property -Object $Report -Name $Name -Context "root"
}

if ($Report.evidence_type -ne "metrics_export_smoke") {
    throw "evidence_type must be 'metrics_export_smoke'. Actual: $($Report.evidence_type)"
}
...
Write-Host "Metrics export report schema validation passed: $ResolvedPath"

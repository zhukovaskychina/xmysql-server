...
if (-not (Test-Path -LiteralPath $Path)) {
    throw "Structured logging report does not exist: $Path"
}

$ResolvedPath = Resolve-Path -LiteralPath $Path
$Report = Get-Content -Raw -LiteralPath $ResolvedPath | ConvertFrom-Json

foreach ($Name in @("generated_at", "run_id", "status", "evidence_type", "repository", "command", "slow_query_log", "error_log", "raw_log", "markdown_report", "exit_code", "checks", "limitations")) {
    Assert-Property -Object $Report -Name $Name -Context "root"
}

if ($Report.evidence_type -ne "structured_logging_smoke") {
    throw "evidence_type must be 'structured_logging_smoke'. Actual: $($Report.evidence_type)"
}
...
Write-Host "Structured logging report schema validation passed: $ResolvedPath"

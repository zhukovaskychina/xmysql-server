...
if (-not (Test-Path -LiteralPath $Path)) {
    throw "Alert drill report does not exist: $Path"
}

$ResolvedPath = Resolve-Path -LiteralPath $Path
$Report = Get-Content -Raw -LiteralPath $ResolvedPath | ConvertFrom-Json

foreach ($Name in @("generated_at", "run_id", "status", "evidence_type", "repository", "alert_rules", "raw_log", "markdown_report", "sample_metrics", "checks", "limitations")) {
    Assert-Property -Object $Report -Name $Name -Context "root"
}

if ($Report.evidence_type -ne "alert_drill_smoke") {
    throw "evidence_type must be 'alert_drill_smoke'. Actual: $($Report.evidence_type)"
}
...
Write-Host "Alert drill report schema validation passed: $ResolvedPath"

...
## 4. CNF 转换器（OPT-006，已并入本优化器文档）

> 原独立文档 `docs/implementation/CNF_CONVERTER_IMPLEMENTATION.md` 已删除，内容归并到本节，避免与「P0 优化器」重复维护。

**代码位置**：`server/innodb/plan/cnf_converter.go`、`cnf_converter_test.go`、`cnf_integration_test.go`；与谓词下推的衔接见 `optimizer.go`（`NewCNFConverter`、`ConvertToCNF`、`ExtractConjuncts` 等）。

**能力摘要**：

- 将 WHERE 中的布尔式转为合取范式（CNF），便于按 AND 子句做谓词下推与索引匹配  
- 双重否定消除、德摩根、NOT 下推、对 AND 分配 OR、子句扁平化与合取/析取项提取  
- 常量折叠、矛盾式检测、子句数/深度上限（防爆栈、爆内存）

**后续**：DNF、更激进的表达式简化仍属扩展项，见下文「未来扩展」。

## 未来扩展

### P1 功能规划

- DNF 表达式转换与更完整的表达式简化规则
- 更复杂的 RBO 规则
- 机器学习增强的 CBO
...
这些 P0 功能与已落地的 **CNF 转换** 共同为后续高级能力（DNF、更强 RBO/CBO 等）提供基础。 